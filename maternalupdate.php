...
<td><button class="view-btn" onclick='openModal(<?php echo json_encode($row); ?>)'>View / Edit</button></td>
</tr>
<?php endforeach; ?>
</tbody>
</table>
...
function openModal(data){
    ...
    // Build modal HTML with all sections
    container.innerHTML = `
    ...
    saveBtn.onclick = () => {
        ...
        'lmp','edc','risk_code','deworming','gravida','term','para','abortion'].forEach(f=>{
            ...
            if(el){
                if(el.type=='checkbox') formData.append(f, el.checked?1:0);
                else formData.append(f, el.value);
            }
        });
