...
<body>
  ...
  <main class="main">
    <section class="content">
      ...
      <form class="fillup_form" method="POST" action="immunizationadd.php">
        ...
        <fieldset>
          <div class="form-grid">
            <div><label>First Name:</label><input type="text" name="first_name" required oninput="capitalizeWords(this)"></div>
            <div><label>Middle Name:</label><input type="text" name="middle_name" oninput="capitalizeWords(this)"></div>
            <div><label>Last Name:</label><input type="text" name="last_name" required oninput="capitalizeWords(this)"></div>
  ...
  <input type="tel" name="contact_number" id="contact_number" placeholder="Enter 7 to 11 digits">
  <small id="contactFeedback" style="color:red; display:block; margin-top:3px;"></small>
</div>
...
contactInput.addEventListener('input', () => {
    ...
    // Check length
    if (contactInput.value.length < 7) {
        feedback.textContent = 'Too short! Must be at least 7 digits.';
    } else if (contactInput.value.length > 11) {
        feedback.textContent = 'Too long! Cannot exceed 11 digits.';
    } else {
